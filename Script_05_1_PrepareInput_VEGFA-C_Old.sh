...
# Read count file
path <- "/Users/seanwen/Documents/Abs/CellPhoneDB/Output/VEGFAC CellPhoneDB_Old/out/"
...
# Rename myeloid progenitors
    # Ligand
    table(df$Ligand.Cluster)
    df$Ligand.Cluster[which(df$Ligand.Cluster=="Myeloid Progenitor 1")] <- "Mono/N"
    df$Ligand.Cluster[which(df$Ligand.Cluster=="Myeloid Progenitor 2")] <- "Eo/Baso/Mast"
    
    # Receptor
    table(df$Receptor.Cluster)
    df$Receptor.Cluster[which(df$Receptor.Cluster=="Myeloid Progenitor 1")] <- "Mono/N"
    df$Receptor.Cluster[which(df$Receptor.Cluster=="Myeloid Progenitor 2")] <- "Eo/Baso/Mast"

# Save file
#path <- "/Users/seanwen/Documents/Abs/CrossTalkeR/Input/"
#file <- "VEGFA-C_Old.csv"
...
# Save file
path <- "/Users/seanwen/Documents/Abs/CrossTalkeR/Input/"
file <- "VEGFA-C_Old_ClustersCollapsed.csv"
...
# Remove myeloid clusters
#index.ligand <- which(df$Ligand.Cluster %in% c("Monocyte/Neutrophil Progenitors", "Eosinophil/Basophil/Mast Cell Progenitors"))
#index.receptor <- which(df$Receptor.Cluster %in% c("Monocyte/Neutrophil Progenitors", "Eosinophil/Basophil/Mast Cell Progenitors"))
#index <- unique(c(index.ligand, index.receptor))
#df <- df[-index, ]

# Save file
#path <- "/Users/seanwen/Documents/Abs/CrossTalkeR/Input/"
#file <- "VEGFA-C_Old_ClustersCollapsed_MyeloidRemoved.csv"
#write.table(df, paste(path, file, sep=""), sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE)
