...
# Read R object
path <- "/Users/khanas/Desktop/scRNAseq_2021/BM_scRNA_count/Count_RProject/SingleCellaR/SingleCellaR_BM/Integration_Files_TKI/"
...
label <- c("ANGPT2", "CD34","FLT4","MCAM","COL4A1","COL4A2","ITGA2","ITGA6","CDC42EP1","DLK1","DLL4","JAM1","JAM3","JAG1","JAG2","KITLG","HBEGF","SEMA3F","TNFS10")
...
if(length(sig.up) != 0 & length(sig.down) != 0) {
  ...
  col.breaks <- c("#e74c3c", "#3498db", "gray")
  
} else if(length(sig.up) != 0 & length(sig.down) == 0) {
  
  col.breaks <- c("#e74c3c", "gray")
  
} else if(length(sig.up) == 0 & length(sig.down) != 0) {
  
  col.breaks <- c("#3498db", "gray")
